...
def smart_money_scores(data, floating):
    ...
    # -----------------------------------------------------
    m = (
        x.set_index("date")["foreign"]
        .resample("ME")
        .sum()
    )

    pr = float((m > 0).mean()) if len(m) else 0
    s2 = ratio_score(pr, 30)
    ...
    if len(f) >= 120:
        prior = float(np.mean(f[-120:-60]))
    elif len(f) > 60:
        prior = float(np.mean(f[:-60]))
    else:
        prior = 0
...
st.caption(
    "엑셀의 A열=날짜, V열=주가, X열=외국인 순매수를 기준으로 분석합니다."
)
...
# 비정상 날짜 제거
today_limit = pd.Timestamp.today() + pd.Timedelta(days=7)
...
if not floating:
    ...
    st.warning(
        "상장주식수를 입력해야 유동주식 대비 매집률과 Smart Money Score가 정확히 계산됩니다."
    )
...
with tabs[1]:
    ...
    fig.update_layout(
        title=f"{stock_name} · 주가와 외국인 일별 순매수",
        height=520,
        hovermode="x unified",
        legend=dict(
            orientation="h"
...
with tabs[2]:
    ...
    fig.update_layout(
        title=f"{stock_name} · 외국인 누적 순매수와 주가",
        height=520,
        hovermode="x unified",
        legend=dict(
            orientation="h"
...
st.caption(
    "※ 엑셀 업로드 기반입니다. A열=날짜 / V열=주가 / X열=외국인 순매수를 사용합니다."
)
